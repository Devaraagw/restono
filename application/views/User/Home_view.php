...
<body class="scroll-bar">
   ...
   <header>
	...
	<script>
			// Fungsi untuk toggle dropdown logout saat ikon profil diklik
			...
			function confirmLogout() {
				...
				if (confirmation) {
					// Menampilkan log bahwa logout berhasil atau tidak
					logLogoutAttempt(true); // Log keberhasilan logout
					window.location.href = "<?php echo site_url('user/member/logout'); ?>"; // Arahkan ke halaman logout
				} else {
					logLogoutAttempt(false); // Log jika logout dibatalkan
				}
			}

			// Fungsi untuk mencatat status logout dalam log
			function logLogoutAttempt(success) {
				var logMessage = success ? 'Logout berhasil' : 'Logout dibatalkan';
				var statusCode = success ? 200 : 400; // Status code untuk log

				// Output ke console log
				console.log(`Status Code: ${statusCode} - ${logMessage}`);
			}
